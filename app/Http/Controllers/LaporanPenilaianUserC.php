...
class LaporanPenilaianUserC extends MyC
{
    ...
    public function getData(Request $request): JsonResponse
    {
        ...
        $totalData = 0;
        $getTotal = $this->LaporanPenilaianUserService->getTotal($sWhere);
        if ($getTotal['status']) {
        ...
        $getData = $this->LaporanPenilaianUserService->getData($sWhere, $sOrder, $sLimit, $cols);
        if ($getData['status']) {
    ...
    public function save(Request $request): JsonResponse
    {
        ...
        $cekValidasi = $this->LaporanPenilaianUserService->validateData($request);
        ...
        if ($request->act == 'edit') {
            $res = $this->LaporanPenilaianUserService->edit($request->cval_id, $data);
        } else {
            $res = $this->LaporanPenilaianUserService->add($data);
        }
    ...
    {
        $res = $this->LaporanPenilaianUserService->del($id);
    ...
    {
        $res = $this->LaporanPenilaianUserService->checkDuplicate($request->act, $request->key, $request->val, (!empty($request->old) ? $request->old : ""));
        return $res;
    }

    public function getById($id): JsonResponse
    {
        $res = $this->LaporanPenilaianUserService->getById($id);
        if ($res['status']) {
            $dt = $res["data"];
            $data = [
                "cval_id" => $dt->cval_id,
