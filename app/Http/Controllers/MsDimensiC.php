...
class MsDimensiC extends MyC
{
    private MsDimensiService $msDimensiService;
    public function __construct(MsDimensiService $msDimensiService)
    {
        parent::__construct();
        $this->middleware("has_akses:ms-dimensi");
        $this->msDimensiService = $msDimensiService;
    }
    ...
    public function getData(Request $request): JsonResponse
    {
        ...
        $totalData = 0;
        $getTotal = $this->msDimensiService->getTotal($sWhere);
        if ($getTotal['status']) {
        ...
        $getData = $this->msDimensiService->getData($sWhere, $sOrder, $sLimit, $cols);
        if ($getData['status']) {
    ...
    public function save(Request $request): JsonResponse
    {
        ...
        $cekValidasi = $this->msDimensiService->validateData($request);
        ...
        if ($request->act == 'edit') {
            $res = $this->msDimensiService->edit($request->md_id, $data);
        } else {
            $res = $this->msDimensiService->add($data);
        }
    ...
    {
        $res = $this->msDimensiService->del($id);
    ...
    {
        $res = $this->msDimensiService->checkDuplicate($request->act, $request->key, $request->val, (!empty($request->old) ? $request->old : ""));
        return $res;
    }

    public function getById($id): JsonResponse
    {
        $res = $this->msDimensiService->getById($id);
        if ($res['status']) {
            $dt = $res["data"];
            $data = [
                "md_id" => $dt->md_id,
